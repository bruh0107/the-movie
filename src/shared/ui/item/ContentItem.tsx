...
interface Props {
    ...
    path: string; // Ожидается 'movie' или 'tv'
}
...
const ContentItem: FC<Props> = ({ content, path }) => {
    ...
    return (
        <div className="group relative w-100 h-130 shrink-0 overflow-hidden">
            <img
                src={content.poster_path ? getStorageUrl(content.poster_path) : '/no-poster.png'}
                className="size-full object-cover transition-all duration-300 rounded-2xl group-hover:brightness-20"
                alt={`item ${content.id}`}
            />

            <div className="absolute inset-0 flex justify-between flex-col opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-white p-4">
